...
fun estimateRealDimensions(
    ...
    val scale: Double? = if (subjectDistance != null) {
        ...
        val centerX =
            (quad.topLeft.x + quad.topRight.x + quad.bottomLeft.x + quad.bottomRight.x) / 4.0
        val centerY =
            (quad.topLeft.y + quad.topRight.y + quad.bottomLeft.y + quad.bottomRight.y) / 4.0
    ...
    val xTL = corner3D(quad.topLeft)
    val xTR = corner3D(quad.topRight)
    val xBR = corner3D(quad.bottomRight)
    val xBL = corner3D(quad.bottomLeft)
