 ...
 * - Harris corner response ([Imgproc.goodFeaturesToTrack] with
...
class PartialShapeDetector(
    ...
    fun detect(frameGray: Mat): PartialShape? {
        ...
        if (lCorners.size >= 4) {
            val quad = assembleQuad(lCorners.map { it.point }, width, height, segments, supportRadius)
            ...
            for (b in a + 1 until corners.size - 2) {
                for (c in b + 1 until corners.size - 1) {
                    for (d in c + 1 until corners.size) {
                        val ordered = createQuad(listOf(corners[a], corners[b], corners[c], corners[d]))
                        ...
                        for (i in points.indices) {
                            ...
                            if (hasConnectingSegment(start, end, segments, supportRadius)) supportedEdges++
                        }
    ...
    private fun buildPartial(
        ...
        lCorners.take(MAX_PARTIAL_CORNERS).forEachIndexed { index, corner ->
            if (index !in linkedIndexes) {
                for (segment in corner.arms) openArms += trimArm(corner.point, segment, maxArmLength)
            }
        }
    ...
    private fun trimArm(corner: Point, segment: Segment, maxLength: Double): Pair<Point, Point> {
        ...
        return if (length <= maxLength || length == 0.0) {
            ...
            corner to Point(corner.x + (far.x - corner.x) * ratio, corner.y + (far.y - corner.y) * ratio)
        }
    }
