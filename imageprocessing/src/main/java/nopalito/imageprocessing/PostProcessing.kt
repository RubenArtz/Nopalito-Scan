...
enum class ColorMode {
    /** Unprocessed: crop + rotation only, no enhancement. */
    ORIGINAL,

    /** Color enhancement (Retinex on L). */
    COLOR,

    /** Enhanced grayscale. */
    GRAYSCALE,

    /** Gentle brightening: lifts brightness/contrast while keeping color. */
    LIGHTEN,
...
fun enhanceCapturedImage(img: Mat, colorMode: ColorMode): Mat {
    return when (colorMode) {
        ColorMode.ORIGINAL -> img.clone()
        ColorMode.COLOR -> multiScaleRetinexOnL(img)
        ColorMode.GRAYSCALE -> enhanceGrayscaleImage(img)
        ColorMode.LIGHTEN -> enhanceLightenImage(img)
        ColorMode.BW -> enhanceBwImage(img)
    }
}

/**
 * Gentle brightening that keeps color: lifts luminance in Lab and stretches
 * whites — cheap and artifact-free.
 */
fun enhanceLightenImage(img: Mat): Mat {
    val lab = Mat()
    ...
    // Lift brightness ~12% and stretch toward white, with clamp.
    Core.add(lF, Scalar(18.0), lF)
    Core.multiply(lF, Scalar(1.06), lF)
    Core.min(lF, Scalar(255.0), lF)
    Core.max(lF, Scalar(0.0), lF)
    lF.convertTo(channels[0], CvType.CV_8U)
    Core.merge(channels, lab)
    val result = Mat()
    Imgproc.cvtColor(lab, result, Imgproc.COLOR_Lab2BGR)
    lab.release(); lF.release()
    channels.forEach { it.release() }
    return result
}

/**
 * High-contrast black & white: grayscale + adaptive threshold for pure text,
 * ideal for printing or removing shadows/marks.
 */
fun enhanceBwImage(img: Mat): Mat {
    val gray = Mat()
    when (img.channels()) {
    ...
    gray.release(); denoised.release(); bw.release()
    return result
}
...
fun multiScaleRetinexOnL(bgr: Mat): Mat {
    ...
    // Expanded kernels: the large kernel (~60% of image) captures large-scale
    // diffuse shadows like the one in Image 1 that the /2 kernel missed.
    ...
    // alpha 0.82 = 82% Retinex, only 18% original. Previously 0.6 left 40% shadow.
    val alpha = 0.82
    Core.addWeighted(
    ...
    // --- 6b. Flatten residual soft shadows (low-frequency illumination) ---
    // Even after Retinex a soft gradient remains (hand/phone shadow).
    // Estimate it with a very large blur and divide: corrected / illumination * mean.
    val illumination = Mat()
    val largeSigma = max(lFloat.cols(), lFloat.rows()).toDouble() / 6.0
    Imgproc.GaussianBlur(correctedL, illumination, Size(0.0, 0.0), largeSigma.coerceAtLeast(35.0))
    Core.max(illumination, Scalar(10.0), illumination)
    val meanIllum = Core.mean(illumination).`val`[0].coerceAtLeast(1.0)
    Core.divide(correctedL, illumination, correctedL, meanIllum, CvType.CV_32F)
    illumination.release()

    // --- 7. Restore contrast + white-point stretch ---
    // Combines percentile stretch with stretching the background peak to 255,
    // same as in enhanceGrayscaleImage. This whitens the paper and removes
    // residual shadow gray.
    val pLowOrig = percentileL(lOriginalFloat, 0.005)
    val pLow = percentileL(correctedL, 0.005)
    val pHigh = percentileL(correctedL, 0.996)

    // Detect background mode (hist peak in [190..255]) to map it to 255.
    val histForMode = Mat()
    Imgproc.calcHist(
        listOf(correctedL), MatOfInt(0), Mat(), histForMode,
        MatOfInt(256), MatOfFloat(0f, 256f)
    )
    var bgMode = -1.0
    var bgCount = -1.0
    if (!histForMode.empty()) {
        val bins = minOf(256, histForMode.rows() * histForMode.cols())
        val counts = FloatArray(bins.coerceAtLeast(1))
        if (histForMode.get(0, 0, counts) > 0) {
            for (i in minOf(190, bins - 1) until bins) {
    ...
    // If there is a clear background peak not already at 255, use it as white-point.
    // Otherwise fallback to classic pHigh.
    val whitePoint = if (bgMode in 190.0..253.0 && bgCount > 0) bgMode else pHigh
    val targetLow = min(pLow, pLowOrig)
    val targetHigh = 255.0
    val scale = (targetHigh - targetLow) / (whitePoint - pLow + 1e-6)
...
/**
 * Safely reads a single CV_32F element from a single-row [mat] (e.g. a sorted
 * 1xN row vector). Returns null when the value cannot be retrieved (empty Mat,
 * out-of-bounds index or depth mismatch).
 *
 ...
 * array" at the [0] indexing site. Row is fixed to 0 because all current
 * callers operate on 1xN sorted vectors.
 */
private fun getFloat(mat: Mat, col: Int): Double? {
    return runCatching {
        val buf = FloatArray(1)
        if (mat.get(0, col, buf) > 0) buf[0].toDouble() else null
    }.getOrNull()
}
...
fun enhanceGrayscaleImage(img: Mat): Mat {
    ...
    // Expanded kernels for large shadows: adds large kernel (~40% of image)
    val kernelSizes = listOf(maxDim / 50, maxDim / 6, maxDim / 2.4)
    val weight = 1.0 / kernelSizes.size
    ...
    val n = sorted.cols()
    val pLow = getFloat(sorted, (n * 0.004).toInt()) ?: 0.0
    val pHigh = getFloat(sorted, (n * 0.99).toInt()) ?: 255.0
    ...
    // Find the histogram mode in [175..255] as an estimate of the background level,
    // then stretch so that level maps to 255. Range expanded to 175 to capture
    // shadowed backgrounds that previously stayed at 175-180 and were not whitened.
    ...
    if (!hist.empty()) {
        ...
        if (hist.get(0, 0, counts) > 0) {
            for (i in minOf(175, bins - 1) until bins) {
                val c = counts[i].toDouble()
                if (c > modeCount) {
                    modeCount = c
                    modeVal = i
    ...
    if (modeVal >= 254) {
        ...
        val gLow = getFloat(graySorted, (gN * 0.01).toInt()) ?: 0.0
        val gHigh = getFloat(graySorted, (gN * 0.99).toInt()) ?: 255.0
