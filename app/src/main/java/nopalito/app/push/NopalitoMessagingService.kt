...
class NopalitoScanMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        // Cloud Link pairing events (Receive scans on your PC, Phase 2):
        // log-only extension point. Phase 4 renders a snackbar and refreshes
        // the file list from here.
        val linkType = data["type"]
        if (linkType == "pc_linked" || linkType == "pc_unlinked" || linkType == "web_upload") {
            Log.i(TAG, "onMessageReceived: cloud-link event=$linkType")
            if (linkType == "pc_unlinked") {
                // The browser (or the user on another device) revoked the
                // pairing: drop the local receipt so the header badge returns
                // to gray instead of showing a stale green dot.
                scope.launch {
                    try {
                        nopalito.app.ui.screens.cloud.data.PcLinkDataStore
                            .getInstance(applicationContext)
                            .clearLinkSession()
                    } catch (e: Exception) {
                        Log.w(TAG, "pc_unlinked: failed to clear pairing: ${e.message}")
                    }
                }
            }
            return
