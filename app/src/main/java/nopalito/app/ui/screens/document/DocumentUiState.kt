...
data class DocumentUiState(
    ...
    val textExtraction: TextExtractionState = TextExtractionState.Idle,
)
...
data class CurrentPageUiState(
    ...
    /**
     * Filter-neutral rendering (ORIGINAL, thumbnail-sized) used as the base
     * for the filter-strip previews, so each cell previews its own filter
     * instead of inheriting the selected one. Null when unavailable — callers
     * fall back to [bitmap].
     */
    val filterPreviewBase: Bitmap? = null,
)
