...
import nopalito.app.ui.screens.tools.deletepages.BlankDetectionParams
...
import nopalito.app.ui.screens.tools.deletepages.DeletePagesEngine
...
/**
 * ViewModel of the "Organize PDF pages" tool: reorder + delete in one place.
 *
 * Combines the two sibling tools: the drag-and-drop reorder of "Reorder PDF
 * pages" and the page deletion of "Delete PDF pages" (trash per thumbnail,
 * batch delete mode, blank-page analysis with progress/cancellation and an undo
 * stack). Everything shares the same architecture: one PDF at a time, a native
 * [PdfRenderer] held open for the lazy preview, and [OrganizerUiState.pageOrder]
 * as the single observable source of truth - reordering moves entries, deleting
 * removes them (undo restores the snapshot), and only the export builds a
 * physical PDF via [DeletePagesEngine.reorderPages] (PDFBox `importPage`,
 * preserving the original text) written through [ToolOutputSaver] into the
 * destination configured in Settings.
 *
 ...
 * recycled.
 */
class OrganizerViewModel(container: AppContainer) : ViewModel() {
    ...
    fun addFile(file: PickedFile) {
        ...
        viewModelScope.launch {
            try {
                ...
                if (withContext(Dispatchers.IO) { isPdfEncrypted(cached) }) {
                    ...
                    _state.update {
                        it.copy(
                            isLoading = false,
                            isLoaded = false,
                            errorMessage = context.stringFor(R.string.org_error_protected, AppLocaleOverride.locale)
                        )
                    }
                    return@launch
                }
                ...
                logger.e("Organizer", "Cannot open PDF", e)
                closeCurrentPdf()
                _state.update {
                    it.copy(
                        isLoading = false,
                        isLoaded = false,
                        errorMessage = context.stringFor(R.string.org_error_invalid, AppLocaleOverride.locale)
                    )
                }
            }
        }
     ...
     * in the observable order. Disabled while the delete mode is active (the
     * strip then marks pages instead of dragging them).
     */
    fun movePage(from: Int, to: Int) {
        val st = _state.value
        if (!st.isLoaded || st.isProcessing || st.deleteMode) return
        val newOrder = DeletePagesEngine.moveElement(st.pageOrder, from, to)
    ...
    private fun applyDeletion(toRemove: Set<Int>) {
        ...
        if (toRemove.size >= st.pageOrder.size) {
            _state.update {
                it.copy(
                    ...
                    deleteDialogVisible = false,
                    results = emptyList(),
                    errorMessage = context.stringFor(R.string.org_cannot_delete_all, AppLocaleOverride.locale),
                )
            }
            return
        }
        ...
        _state.update {
            it.copy(
                pageOrder = newOrder,
                selectedPreviewIndex = it.selectedPreviewIndex.coerceIn(0, newOrder.lastIndex.coerceAtLeast(0)),
                undoStack = (it.undoStack + OrganizerUndoEntry(
    ...
    fun startBlankAnalysis() {
        ...
        blankAnalysisJob = viewModelScope.launch {
            try {
                ...
                logger.e("Organizer", "Blank analysis failed", e)
                _state.update {
                    it.copy(
                        isAnalyzingBlanks = false,
                        blankProgress = null,
                        blankProgressLabel = null,
                        errorMessage = context.stringFor(R.string.org_blank_analysis_failed, AppLocaleOverride.locale),
                    )
                }
            }
        }
    ...
    fun exportPdf() {
            ...
            _state.update {
                it.copy(
                    errorMessage = context.stringFor(
                        R.string.org_nothing_to_export,
                        AppLocaleOverride.locale
                    )
                )
            }
        ...
        if (st.passwordEnabled && st.password.isBlank()) {
            _state.update {
                it.copy(
                    errorMessage = context.stringFor(
                        R.string.org_password_required,
                        AppLocaleOverride.locale
                    )
                )
            }
        ...
        if (!destinationAvailable(st.saveLocationUri)) {
            _state.update {
                it.copy(
                    errorMessage = context.stringFor(
                        R.string.org_destination_unavailable,
                        AppLocaleOverride.locale
                    )
                )
            }
        ...
        _state.update {
            it.copy(
                isProcessing = true,
                progress = 0f,
                progressLabel = context.stringFor(R.string.org_progress, AppLocaleOverride.locale),
                results = emptyList(),
                errorMessage = null,
            )
        }
        viewModelScope.launch {
            try {
                ...
                logger.e("Organizer", "PDF export failed", e)
                _state.update {
                    it.copy(
                        isProcessing = false,
                        progress = null,
                        progressLabel = null,
                        errorMessage = context.stringFor(R.string.org_error, AppLocaleOverride.locale)
                    )
                }
            }
        }
    ...
    private suspend fun exportOnIo(st: OrganizerUiState): Pair<OrganizerResult, ToolOutputSaver.SaveRef> {
        val input = currentFile ?: throw IllegalStateException("No PDF loaded")
        val reordered = File(toolsDir, "organized_${System.currentTimeMillis()}.pdf")
        ...
        try {
            ...
            DeletePagesEngine.reorderPages(input, orderIndices, reordered, password) { done, total ->
     ...
     */
    private suspend fun recordInHistory(result: OrganizerResult, ref: ToolOutputSaver.SaveRef) {
        ...
        try {
            ...
            logger.e("OrganizerHistory", "Failed to record organizer history", e)
        }
    }
