...
class PasswordProtectViewModel(container: AppContainer) : ViewModel() {
    ...
    fun reportInvalidFileType() {
        ...
        _state.update {
            it.copy(
                errorMessage = context.stringFor(R.string.pp_invalid_file_type, AppLocaleOverride.locale, typeName),
                results = emptyList(),
            )
        }
    }
    ...
    fun save() {
        ...
        when {
            st.password.isBlank() -> _state.update {
                it.copy(errorMessage = context.stringFor(R.string.pp_password_required, AppLocaleOverride.locale))
            }

            !destinationAvailable(st.saveLocationUri) -> _state.update {
                it.copy(errorMessage = context.stringFor(R.string.pp_destination_unavailable, AppLocaleOverride.locale))
            }

            else -> {
                _state.update { it.copy(isProcessing = true, results = emptyList(), errorMessage = null) }
                viewModelScope.launch {
                    try {
    ...
    private suspend fun protectAll(
        ...
        val processed = st.files.map { file ->
            ...
            val outputRef = saver.save(targetName, protectedFile, st.saveLocationUri, batchFolder?.uri)
            val finalName = (outputRef as? ToolOutputSaver.SaveRef.FileRef)?.file?.name ?: targetName
    ...
    private fun mapError(e: Exception): String = when (e) {
        is InvalidPasswordException -> context.stringFor(R.string.pp_error_protected, AppLocaleOverride.locale)
