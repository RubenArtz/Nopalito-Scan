    ...
    fun parse(input: String, totalPages: Int): PageRangeResult {
        ...
        for (token in tokens) {
            ...
            if (explicit != null) {
                ...
                if (b != totalPages) warnings += PageRangeWarning(statedTotal = b, actualTotal = totalPages)
            ...
            if (range != null) {
                ...
                if (start > end) return invalid(PageRangeErrorKind.DESCENDING, token, a = start, b = end)
                pages += start..end
                continue
            }
