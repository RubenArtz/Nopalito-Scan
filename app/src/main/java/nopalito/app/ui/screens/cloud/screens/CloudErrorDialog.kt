    ...
    return when (e.code) {
        ApiException.ACCOUNT_NOT_FOUND -> CloudAuthDialog(
            application.stringFor(
                R.string.cloud_dialog_account_not_found_title,
                AppLocaleOverride.locale
            ),
            application.stringFor(
                R.string.cloud_dialog_account_not_found_body,
                AppLocaleOverride.locale
            )
        )
        ...
        ApiException.AUTH_LOGIN_BLOCKED_SUSPENDED,
        ...
        ApiException.AUTH_FORBIDDEN_ACCOUNT_DELETED,
        ApiException.AUTH_FORBIDDEN_ACCOUNT_INACTIVE_DELETED -> CloudAuthDialog(
            application.stringFor(
                R.string.cloud_dialog_account_deleted_title,
                AppLocaleOverride.locale
            ),
            application.stringFor(
                R.string.cloud_dialog_account_deleted_body,
                AppLocaleOverride.locale
            )
        )

        ApiException.AUTH_TOO_MANY_DEVICES -> {
            val maxDevices =
                nopalito.app.ui.screens.cloud.model.ApiDetails.getInt(e.details, "maxDevices")
                    ?.toString()
                    ?: nopalito.app.ui.screens.cloud.model.ApiDetails.getInt(e.details, "max")
                        ?.toString()
                    ?: "5"
            val bodyTemplate = application.stringFor(
                R.string.cloud_dialog_too_many_devices_body,
                AppLocaleOverride.locale
            )
            val body = if (bodyTemplate.contains("%1\$s")) bodyTemplate.replace(
                "%1\$s",
                maxDevices
            ) else bodyTemplate
            CloudAuthDialog(
                application.stringFor(
                    R.string.cloud_dialog_too_many_devices_title,
                    AppLocaleOverride.locale
                ),
                body
            )
