...
fun MagnifyingGlass(
    ...
    val bitmapRegionHalf =
        (bitmap.width / displaySize.width.toFloat()) * configPx.loupeRadius / ZOOM_FACTOR
    ...
    Canvas(
        ...
        clipPath(circlePath) {
            ...
            if (srcRight > srcLeft && srcBottom > srcTop) {
                ...
                val dstWidth =
                    ((srcRight - srcLeft) / (2 * bitmapRegionHalf) * loupeDiameter).toInt()
                val dstHeight =
                    ((srcBottom - srcTop) / (2 * bitmapRegionHalf) * loupeDiameter).toInt()
...
internal fun computeLoupeCenter(
    ...
    if (aboveCenterY - loupeRadius >= screenMargin) {
        ...
        val cx = dragPosition.x.coerceIn(
            screenMargin + loupeRadius,
            containerWidth - screenMargin - loupeRadius
        )
