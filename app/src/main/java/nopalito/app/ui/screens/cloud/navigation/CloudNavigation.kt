...
import nopalito.app.ui.screens.cloud.navigation.CloudDeepLink.consume
...
sealed class CloudScreen {
    ...
    /** Session recovery — release all active sessions via email OTP (unauthenticated only). */
    object SessionRecovery : CloudScreen()

    /** Server-authoritative storage usage (plan + quota + progress bar). */
    object Storage : CloudScreen()

    /** Plain file upload; [parentFolderId] targets the browsed folder (null = root). */
    ...
    object Sessions : CloudScreen()

    /** PC pairing (Receive scans on your PC): QR scan + PIN entry + devices. */
    object PcLink : CloudScreen()
}