 ...
 *
 * Never used authenticators here: `BIOMETRIC_WEAK` is rejected at the
 * capability layer, and `DEVICE_CREDENTIAL` is only armed on API 30+ where it
 * can actually release a crypto key (see [BiometricCapability]).
 *
...
class BiometricTokenStore internal constructor(
    ...
    /** Idempotent: creates the Keystore key if it does not exist yet. */
