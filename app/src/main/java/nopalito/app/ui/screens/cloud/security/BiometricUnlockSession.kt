...
class BiometricUnlockSession(
    ...
    fun rotateRefreshToken(newRefresh: String): Boolean {
        ...
        val ct = runCatching { aesGcm(t2, iv, newRefresh.toByteArray(), Cipher.ENCRYPT_MODE) }.getOrNull()
            ?: return false
    ...
    private fun aesGcm(tier2: ByteArray, iv: ByteArray, input: ByteArray, mode: Int): ByteArray {
        ...
        cipher.init(mode, SecretKeySpec(tier2, KEY_ALGORITHM), GCMParameterSpec(GCM_TAG_LENGTH_BITS, iv))
