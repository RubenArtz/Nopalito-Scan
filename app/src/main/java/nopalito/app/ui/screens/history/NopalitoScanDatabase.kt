...
import nopalito.app.data.stats.StatsDao
import nopalito.app.data.stats.StatsEvent
import nopalito.app.ui.screens.qr.QrScanDao
import nopalito.app.ui.screens.qr.QrScanEntity

@Database(
    entities = [ExportHistoryEntity::class, QrScanEntity::class, StatsEvent::class],
    version = 9,
    exportSchema = false
)
abstract class NopalitoScanDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /** v8 -> v9: stats events for usage statistics (MVP 100% local). */
        val MIGRATION_8_9 = object : Migration(8, 9) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                db.execSQL("CREATE INDEX IF NOT EXISTS index_stats_events_timestamp ON stats_events(timestamp)")
                db.execSQL("CREATE INDEX IF NOT EXISTS index_stats_events_eventType ON stats_events(eventType)")
                db.execSQL("CREATE INDEX IF NOT EXISTS index_stats_events_timestamp_eventType ON stats_events(timestamp, eventType)")
            }
        }
