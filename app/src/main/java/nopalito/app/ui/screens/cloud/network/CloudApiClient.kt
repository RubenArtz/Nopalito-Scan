...
class CloudApiClient private constructor(context: Context) {
    ...
    private val okHttpClient: OkHttpClient by lazy {
        ...
        // 1. Metrics FIRST: covers the whole wall time including auth refresh
        //    and every retry attempt (structured endpoint/duration/status log).
        builder.addInterceptor(HttpMetricsInterceptor())
        ...
        // onForceLogout wipes the biometric blob on definitive server-driven logout
        // (401 refresh rejected / 403 suspended/deleted) so the app truly asks for
        // login next time. Transient network failures (timeout, UnknownHost, 5xx)
        // never trigger it — they keep the blob.
        builder.addInterceptor(
            AuthInterceptor(
                tokenProvider = tokenProvider,
                refreshApi = lazy { refreshAuthApi },
                biometricSessionManager = biometricSessionManager,
                onForceLogout = {
                    if (biometricSessionManager.isEnabled) {
                        ...
                        android.util.Log.w(
                            "CloudApiClient",
                            "AuthInterceptor force-logout → biometric blob wiped"
                        )
                    }
                }
            )
        ...
        // 2. Limited transient-error retries with exponential backoff.
        //    Placed AFTER auth so a retry does not re-trigger the refresh
        //    flow, and BEFORE debug logging so every attempt is visible.
        builder.addInterceptor(
            RetryInterceptor(maxAttempts = RetryInterceptor.DEFAULT_MAX_ATTEMPTS)
        )
    ...
    /** User preferences + account status (language sync, lifecycle). */
    val userPreferences: UserPreferencesApi by lazy {
        createService(UserPreferencesApi::class.java)
    }

    /** Billing (Google Play, legacy 4 productIds, prices via formattedPrice only). */
    val billingApi: BillingApi by lazy {
        createService(BillingApi::class.java)
    }
    ...
    /** Cloud Link pairing approval + linked browsers (Receive scans on PC). */
    val cloudLinkApi: CloudLinkApi by lazy {
        createService(CloudLinkApi::class.java)
    }

    val avatarApi: AvatarApi by lazy {
        createService(AvatarApi::class.java)
    }
    ...
    private fun validateBaseUrl(url: String): String {
        ...
        // Release builds must use HTTPS (fail-closed); debug allows http for local dev (192.168.x)
