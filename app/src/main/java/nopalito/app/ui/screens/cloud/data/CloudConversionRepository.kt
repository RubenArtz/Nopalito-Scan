...
class CloudConversionRepository(private val context: Context) {
    ...
    suspend fun startConversion(files: List<Pair<Uri, String>>): Result<ConversionJobData> {
        ...
        return try {
            // Copying picked content:// streams is blocking IO — keep it off
            // the caller's (usually Main) dispatcher.
            val parts = withContext(Dispatchers.IO) {
                files.map { (uri, fileName) ->
                    val tempFile =
                        File(
                            context.cacheDir,
                            "conv_upload_${System.currentTimeMillis()}_${fileName.hashCode()}"
                        )
            ...
            if (response.isSuccessful) {
                ...
                if (body != null && body.success && body.data != null) {
                    ...
                    Result.failure(
                        ApiException(
                            body?.error?.code,
                            body?.message ?: context.stringFor(
    ...
    suspend fun downloadConverted(jobId: String, fileId: String, fileName: String): Result<File> {
        return try {
            ...
            val target =
                File(context.cacheDir, "conv_download_${System.currentTimeMillis()}_$fileName")
