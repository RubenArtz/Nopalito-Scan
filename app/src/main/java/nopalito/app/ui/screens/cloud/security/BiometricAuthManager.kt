 ...
 *
 * Never uses `BIOMETRIC_WEAK`, never writes the plaintext token, and re-creates
 * the Keystore key if [KeyPermanentlyInvalidatedException] shows up while
 * enabling (biometrics changed since last time).
 */
class BiometricAuthManager(
    ...
    fun authenticate(request: BiometricRequest, listener: (BiometricAuthResult) -> Unit) {
        when (val prepared = prepareCipher(request)) {
            ...
            PreparedCipher.NoCryptoPrompt -> {
                Log.d(TAG, "prompt: crypto-less fallback (cipher could not be prepared before auth)")
     ...
     */
    private fun createEncryptCipherWithRetry(): Result<Cipher> {
        var result = store.createEncryptCipher()
        if (result.exceptionOrNull() is BiometricTokenError.KeyInvalidated) {
            Log.d(TAG, "cipher create: KeyInvalidated → fresh key and retry")
            store.deleteKey()
            try {
    ...
    private fun prepareCipher(request: BiometricRequest): PreparedCipher = when (request) {
        is BiometricRequest.Encrypt -> {
            ...
            val result = createEncryptCipherWithRetry()
            if (result.isSuccess) {
                ...
                Log.w(TAG, "cipher create: fail, exception=${error?.javaClass?.simpleName ?: "unknown"}")
                when (error) {
        ...
        BiometricRequest.Decrypt -> {
            ...
            when (val error = result.exceptionOrNull()) {
                ...
                BiometricTokenError.StorageUnavailable,
                    -> {
                    Log.w(TAG, "cipher create: fail (decrypt), exception=${error::class.simpleName}")
                    PreparedCipher.NoCryptoPrompt
                }
    ...
    private fun onPromptSuccess(
        ...
        when (request) {
            is BiometricRequest.Encrypt -> {
                ...
                val active = cipher ?: createEncryptCipherWithRetry().getOrElse { e ->
            ...
            BiometricRequest.Decrypt -> {
                ...
                val active = cipher ?: store.createDecryptCipher().getOrElse { e ->
                    Log.w(TAG, "cipher create: fail after auth (decrypt), exception=${e::class.simpleName}")
                    listener(
                        when (e) {
