...
class AndroidDocxWriter(
    ...
    // Option B (opt-in). Default null = Option A, existing DI untouched.
    ...
    private suspend fun runOcrLines(
        ...
        return try {
            // Same robust pipeline as PDF: detector when present (B),
            // otherwise internal preprocessing + SINGLE_BLOCK (A).
            val rawBoxes = if (ocrDetector != null) {
                ocrDetector.detect(bitmap)
            } else {
                val pre = runCatching { ocrService.preprocessForOcr(bitmap) }
                    .getOrNull() ?: bitmap
                try {
                    ocrService.runOcr(pre, TessBaseAPI.PageSegMode.PSM_SINGLE_BLOCK)
                } finally {
                    if (pre !== bitmap && !pre.isRecycled) pre.recycle()
                }
            }
            // Export filter before grouping: drops 1-char garbage paragraphs.
            val clean = filterBoxesForExport(rawBoxes, bitmap.height)
            groupWordsIntoLines(clean, bitmap.height)
...
/**
 * Groups OCR words into paragraphs ordered from top to bottom. Each returned
 * string is one PARAGRAPH whose visual lines are joined with "\n" (split back
 * into <w:br/> by buildTextParagraph). Delegates to the shared
 * [OcrTextExtractor] so the editor text extraction and the DOCX exporter use
 * the same algorithm.
 */
internal fun groupWordsIntoLines(
    ...
    // DocxPageMedia.ocrLines and all callers are unchanged: entries are just
    // paragraphs now instead of single lines.
    val lines = OcrTextExtractor().extractWithStructure(textBoxes, imageHeightPx)
    val paragraphs = mutableListOf<MutableList<String>>()
    for (line in lines) {
        if (line.startsNewParagraph || paragraphs.isEmpty()) paragraphs += mutableListOf(line.text)
        else paragraphs.last() += line.text
    }
    return paragraphs.map { it.joinToString(separator = "\n") }
}
...
internal object DocxFileBuilder {
    ...
    private fun buildTextParagraph(
        ...
        // Each entry is one paragraph; embedded "\n" are soft line breaks.
        val runs = text.split("\n").map { xmlEscape(cleanXmlText(it)) }

        return buildString {
            append("<w:p>")
            // Wider after-spacing marks real paragraphs; lines inside share one.
            append("<w:pPr><w:spacing w:after=\"200\"/></w:pPr>")
            append("<w:r>")
