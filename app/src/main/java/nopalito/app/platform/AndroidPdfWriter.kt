            ...
            for ((index, page) in pages.withIndex()) {
                ...
                if (baseBitmap != null) {
                    ...
                    if (composedBitmap != null) {
                        try {
                            val jpegBytes = bitmapToJpegBytes(composedBitmap, 85)
                ...
                if (!disableOcr && baseBitmap != null) {
                    try {
                        // Reuse already-decoded baseBitmap; OcrService internally
                        // downscales to ~1.2MP so Tesseract takes 2-4s instead of
                        // 60s+ on full-res BALANCED/HIGH images.
                        val ocrTextBoxes = ocrService.runOcr(baseBitmap)
