                ...
                if (backgroundStyle != DateBackgroundStyle.NONE) {
                    drawRoundRect(
                        RectF(
                            boxLeft,
                            boxTop,
                            boxLeft + metrics.widthPx,
                            boxTop + metrics.heightPx
                        ),
                        metrics.cornerRadiusPx,
                        metrics.cornerRadiusPx,
                        bgPaint,
...
class AndroidPdfWriter(
    val ocrService: OcrService,
    val assets: AssetManager,
    // Option B (opt-in): when injected, export uses the same pipeline as the
    // preview (binarize + SINGLE_BLOCK). When null (default) it uses Option A:
    // OcrService internal preprocessing. Default null keeps existing DI intact.
...
class OcrDocument(
    ...
    private fun buildTextStream(
        ...
        // Order into lines first: iterator order is not guaranteed
        // top-to-bottom, and copy-paste follows emission order. One BT/ET per
        // line (instead of per word) makes readers extract faithful rows, and
        // the larger inter-paragraph y-gaps surface as paragraph breaks.
        val lines = OcrTextExtractor().groupIntoLines(ocrTextBoxes)
        for (line in lines) {
            if (line.isEmpty()) continue
            val words = line.sortedBy { it.box.left }
            val lineLeft = words.minOf { it.box.left }
            val lineRight = words.maxOf { it.box.right }
            val lineBottom = words.maxOf { it.lineBottom }
            val sortedH = words.map { it.lineHeight }.sorted()
            val medianH = sortedH[sortedH.size / 2]
            val x = lineLeft * scaleX
            val lineWidth = (lineRight - lineLeft) * scaleX
            // Clamp outliers: a single bad box used to emit deformed glyphs.
            val fontSize = (medianH * scaleY * 0.8f).coerceIn(4f, 48f)
            val text = words.joinToString(separator = " ") { it.text.trim() }
            if (text.isEmpty()) continue
            // nominal width: fontSize / kCharWidth (Tesseract convention)
            val nominalWidth = text.length * fontSize / 2f
            val hScale = (if (nominalWidth > 0f) (lineWidth / nominalWidth) * 100f else 100f)
                .coerceIn(30f, 300f)
            val baselineY = dimensions.pageHeight - lineBottom * scaleY + fontSize * 0.2f
