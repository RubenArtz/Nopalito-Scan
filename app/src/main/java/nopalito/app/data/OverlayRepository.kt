...
class OverlayRepository(
    ...
    suspend fun loadAll(): Map<String, PageOverlays> = withContext(Dispatchers.IO) {
        ...
        for ((pageId, p) in persisted.pages) {
            ...
            val posFraction = if (p.signaturePositionFractionX != null && p.signaturePositionFractionY != null) {
                androidx.compose.ui.geometry.Offset(p.signaturePositionFractionX, p.signaturePositionFractionY)
            } else null

            val datePosFraction = if (p.datePositionFractionX != null && p.datePositionFractionY != null) {
                androidx.compose.ui.geometry.Offset(p.datePositionFractionX, p.datePositionFractionY)
            } else null
