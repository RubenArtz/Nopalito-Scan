...
class CloudAuthDialogTest {
    ...
    fun suspendedAccountMapsToSuspensionDialog() {
        val dialog = authAccountDialog(ApiException(ApiException.AUTH_ACCOUNT_SUSPENDED, "x"), application)
    ...
    fun accountNotFoundStillMapsToItsOwnDialog() {
        val dialog = authAccountDialog(ApiException(ApiException.ACCOUNT_NOT_FOUND, "x"), application)
        assertNotNull(dialog)
    ...
    fun registerBlockCodesStayInline() {
        assertNull(authAccountDialog(ApiException(ApiException.AUTH_REGISTER_IP_LIMIT_REACHED, "x"), application))
        assertNull(authAccountDialog(ApiException(ApiException.AUTH_REGISTER_VPN_NOT_ALLOWED, "x"), application))
        assertNull(authAccountDialog(ApiException(ApiException.AUTH_ACCOUNT_STATUS_UNKNOWN, "x"), application))
    }
