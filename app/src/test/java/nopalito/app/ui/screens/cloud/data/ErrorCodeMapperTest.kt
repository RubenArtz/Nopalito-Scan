...
class ErrorCodeMapperTest {
    ...
    fun specificCodeMapsToItsDedicatedResource() {
        assertEquals(ErrorCodeMapper.resolveResId("QUOTA_EXCEEDED", 400), R.string.cloud_error_quota_exceeded)
        assertEquals(
    ...
    fun rateLimitGroupMapsToTooManyRequests() {
        assertEquals(ErrorCodeMapper.resolveResId("RATE_LIMIT_EXCEEDED", 500), R.string.cloud_error_rate_limit)
    }

    @Test
    fun migrationGroupMapsToBadRequest() {
        assertEquals(ErrorCodeMapper.resolveResId("INVALID_MIGRATION_ID", 404), R.string.cloud_error_migration)
        assertEquals(ErrorCodeMapper.resolveResId("MIGRATION_STALLED", 500), R.string.cloud_error_migration)
    }

    @Test
    fun maintenanceGroupMapsToServiceUnavailable() {
        assertEquals(ErrorCodeMapper.resolveResId("MAINTENANCE_ACTIVE", 400), R.string.cloud_error_maintenance_active)
    }

    @Test
    fun qrGroupMapsToBadRequest() {
        assertEquals(ErrorCodeMapper.resolveResId("QR_DATA_TOO_LARGE", 500), R.string.cloud_error_qr)
    }
    ...
    fun groupBeatsStatus() {
        ...
        assertEquals(ErrorCodeMapper.resolveResId("JOB_FILE_NOT_FOUND", 500), R.string.cloud_error_job)
    }

    @Test
    fun groupBeatsStatusWhenNoSpecificMatch() {
        assertEquals(ErrorCodeMapper.resolveResId("QUOTA_SOMETHING", 503), R.string.cloud_error_quota)
    }
    ...
    fun quotaPlaceholdersAreSubstituted() {
        ...
        val applied = ErrorCodeMapper.apply(
            ErrorCodeMapper.format("{usedBytes}/{quotaBytes} ({availableBytes} free)", details).pattern,
    ...
    fun missingPlaceholderIsLeftUntouched() {
        ...
        assertEquals("Generic {count} error", ErrorCodeMapper.apply(formatted.pattern, formatted.args))
    }
