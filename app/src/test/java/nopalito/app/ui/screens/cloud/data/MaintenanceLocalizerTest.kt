...
class MaintenanceLocalizerTest {
    ...
    fun allowedKeysMatchTheDocumentedWhitelist() {
        assertEquals(
            setOf("maintenance.scheduled_title", "maintenance.emergency_title", "maintenance.upgrade_title"),
            MaintenanceLocalizer.allowedTitleKeys
        )
        assertEquals(
            setOf("maintenance.scheduled_message", "maintenance.emergency_message", "maintenance.upgrade_message"),
            MaintenanceLocalizer.allowedMessageKeys
        )
        assertEquals(
            setOf("maintenance.scheduled_reason", "maintenance.emergency_reason", "maintenance.upgrade_reason"),
    ...
    fun upgradeCodeMapsPerField() {
        assertEquals(R.string.cloud_maint_upgrade_title, resource(resolve("maintenance.upgrade", null, null, TITLE)))
        assertEquals(
            R.string.cloud_maint_upgrade_message,
            resource(resolve("maintenance.upgrade", null, null, MESSAGE))
        )
        assertEquals(R.string.cloud_maint_upgrade_reason, resource(resolve("maintenance.upgrade", null, null, REASON)))
    }
    ...
    fun legacyAlwaysSanitized() {
        val r = resolve(null, null, "<b>hi</b> secret=abc https://evil.com/x C:\\app\\file.txt", TITLE)
        assertTrue(r is ResolvedField.Legacy)
    ...
    fun genericResourceUsedWhenNothingProvided() {
        ...
        assertEquals(R.string.cloud_maint_generic_message, resource(resolve(null, null, null, MESSAGE)))
        assertEquals(R.string.cloud_maint_generic_reason, resource(resolve(null, null, null, REASON)))
    }
    ...
    fun unknownKeyWithoutAnythingFallsToGeneric() {
        assertEquals(R.string.cloud_maint_generic_title, resource(resolve(null, "not.a.valid.key", null, TITLE)))
    }
    ...
    fun blankLegacyFallsThroughToGeneric() {
        assertEquals(R.string.cloud_maint_generic_message, resource(resolve(null, null, "", MESSAGE)))
        assertEquals(R.string.cloud_maint_generic_message, resource(resolve(null, null, "   \t ", MESSAGE)))
    }

    @Test
    fun blankLegacyWithUnknownCodeFallsToGeneric() {
        assertEquals(R.string.cloud_maint_generic_reason, resource(resolve("maintenance.other", null, "", REASON)))
    }
