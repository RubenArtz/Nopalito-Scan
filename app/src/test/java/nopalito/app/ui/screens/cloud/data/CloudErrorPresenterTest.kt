...
class CloudErrorPresenterTest {
    ...
    fun codedErrorNeverFallsBackToBackendMessage() {
        ...
        val resolved = CloudErrorPresenter.resolve(e, R.string.error_unknown)
        assertTrue(
            "coded error must resolve to a resource, not Legacy",
            resolved is CloudErrorPresenter.Resolved.Res
        )
    }
    ...
    fun sanitizeStripsUrlsTokensAndPaths() {
        val dirty =
            "Failed https://example.com/auth?t=1 Bearer eyJhbGci token and /home/user/file.txt"
