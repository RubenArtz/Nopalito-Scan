...
class Stage8FStructuralTest {
    ...
    fun qrGeneratorViewModelUsesLocalizedErrors() {
        ...
        assertTrue(
            ...
            text.contains("R.string.qr_content_required")
        )
    ...
    fun maintenanceViewModelLogsAreSafe() {
        val text =
            sourceText("nopalito/app/ui/screens/cloud/viewmodel/CloudMaintenanceViewModel.kt")
    ...
    fun historyScreensUseLocaleAwareDateFormats() {
        for (path in listOf(
            ...
            assertTrue(
                "$path must use the locale-aware getDateTimeInstance",
                text.contains("getDateTimeInstance")
            )
        }
    }
    ...
    fun trashCountdownIsFullyLocalized() {
        ...
        assertTrue(
            "must format dates via ofLocalizedDateTime",
            text.contains("ofLocalizedDateTime")
        )
    }
    ...
    fun cloudFileSizeUsesLocaleStableDecimalFormat() {
        ...
        assertTrue(
            "must use String.format with Locale.US",
            text.contains("String.format(Locale.US")
        )
    }
