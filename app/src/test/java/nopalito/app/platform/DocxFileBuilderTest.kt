...
class DocxFileBuilderTest {
    ...
    @Test
    fun `group words into paragraphs ordered top to bottom`() {
        val words = listOf(
        ...
        // Tight gap (5px < 1.6x20px): one paragraph, lines joined with "\n".
        val lines = groupWordsIntoLines(words)
        assertEquals(listOf("word1 word2 word3\nnextLine"), lines)
    }

    @Test
    fun `wide vertical gap opens a new paragraph`() {
        val words = listOf(
            OcrTextBox("para1", ImageRect(10, 5, 60, 25), 20, 25),
            OcrTextBox("para2", ImageRect(10, 100, 60, 120), 20, 120),
        )
        val lines = groupWordsIntoLines(words)
        assertEquals(listOf("para1", "para2"), lines)
    }
