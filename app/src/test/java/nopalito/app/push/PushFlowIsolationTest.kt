...
class PushFlowIsolationTest {
    ...
    fun pushClassesNeverReferenceMaintenanceLocalizer() {
        for (path in pushClasses) {
            ...
            assertFalse("$path references MaintenanceLocalizer", text.contains("MaintenanceLocalizer"))
        }
    }
    ...
    fun pushClassesNeverUseTheMaintenanceSanitizer() {
        for (path in pushClasses) {
            val text = sourceText(path)
            assertFalse("$path uses sanitizeMaintenanceText", text.contains("sanitizeMaintenanceText"))
            assertFalse("$path uses localizeMaintenanceText", text.contains("localizeMaintenanceText"))
    ...
    fun pushServiceUsesNotificationSanitizationPipeline() {
        val service = sourceText("nopalito/app/push/NopalitoMessagingService.kt")
        assertTrue("service must use localizeNotificationText", service.contains("localizeNotificationText"))
        assertTrue("service must use sanitizeNotificationText", service.contains("sanitizeNotificationText"))
    }
    ...
    fun notificationSanitizerDelegatesToSharedCore() {
        ...
        assertTrue("sanitizeNotificationText must delegate to the shared core", text.contains("sanitizeUntrustedText"))
    }
}