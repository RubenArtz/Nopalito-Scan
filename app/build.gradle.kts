...
android {
    ...
    compileSdk = 37
    // Pin the NDK present in the SDK so packaging can strip native libs
    // (without it every .so ships unstripped with an "Unable to strip"
    // warning). Check installed versions under $ANDROID_HOME/ndk.
    ...
    defaultConfig {
        ...
        // Release must be https (fail-closed for A5)
        if (apiBaseUrl.startsWith("http://") && System.getenv("ALLOW_HTTP_RELEASE") != "true") {
            // Allow http only for debug/local; release enforces https via Gradle check + runtime validateBaseUrl
            // This is a soft warning at build time — runtime will hard-fail in release.
        }
        buildConfigField("String", "API_BASE_URL", "\"$apiBaseUrl\"")

        // Google Sign-In server client ID (Web OAuth client). Empty string by
        // default so builds without Google still succeed; runtime checks the value.
        val googleServerClientId =
            (findProperty("GOOGLE_SERVER_CLIENT_ID") as String?)?.takeIf { it.isNotBlank() }
                ?: Properties().let {
                    val f = rootProject.file("local.properties")
                    if (f.exists()) {
                        f.inputStream().use { s -> it.load(s) }
                    }
                    it.getProperty("GOOGLE_SERVER_CLIENT_ID")
...
dependencies {
    ...
    // alongside litert-support duplicates namespaces, mixes native libs and
    // drops org.tensorflow.lite.Delegate (see proguard-rules.pro).
    ...
    // Google Play Billing — subscriptions (legacy 4 productIds: nopalito_personal_monthly etc.)
    // Prices are never hardcoded; formattedPrice from ProductDetails is the only source.
    ...
    // Google Sign-In: Credential Manager (primary) + Play Services Auth fallback
